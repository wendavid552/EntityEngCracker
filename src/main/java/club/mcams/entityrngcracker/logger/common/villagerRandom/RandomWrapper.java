...
import club.mcams.entityrngcracker.mixins.IRandomAccessor;
...
public class RandomWrapper extends
        //#if MC<11904
        Random
        //#else
        //$$ CheckedRandom
        //#endif
{
    VillagerRandomLogHelper helper;
    private final Random random;

    public RandomWrapper(Random random, VillagerRandomLogHelper helper) {
        //#if MC>=11904
        //$$ super(((IRandomAccessor)random).getSeed().get());
        //#endif
        this.random = random;
        this.helper = helper;
    }
    ...
    public int next(int bits) {
        AtomicLong seed = ((IRandomAccessor)random).getSeed();
        long oldSeed = seed.get();
        int result = ((IRandomAccessor)random).invokeNext(bits);
        long newSeed = seed.get();
        helper.onRandomChanged(bits, oldSeed, newSeed);
        return result;
    }
}
