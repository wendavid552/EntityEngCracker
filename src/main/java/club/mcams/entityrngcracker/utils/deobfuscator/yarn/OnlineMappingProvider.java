...
package club.mcams.entityrngcracker.utils.deobfuscator.yarn;

import club.mcams.entityrngcracker.CrackerMod;
import club.mcams.entityrngcracker.CrackerServer;
import club.mcams.entityrngcracker.utils.FileUtil;
import club.mcams.entityrngcracker.utils.MiscUtil;
...
{
	private static final Logger LOGGER = CrackerServer.LOGGER;
	...
	public static final String STORAGE_DIRECTORY = String.format("./config/%s/mapping/", CrackerMod.compactName);
	...
	synchronized private static String getYarnVersion(boolean useCache) throws IOException
	{
		...
		if (FileUtil.isFile(file))
		{
			...
			{
				CrackerServer.LOGGER.warn("Failed to deserialize data from {}: {}", YARN_VERSION_CACHE_FILE, e);
			}
			if (caches != null)
			{
...
//			MiscUtil.startThread("Yarn Version Updater", () -> checkYarnVersionUpdate(yarnVersion));
	...
	{
		final String MAPPING_STORAGE_DIRECTORY_OLD = String.format("./config/%s/", CrackerMod.compactName);
