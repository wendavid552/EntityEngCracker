...
import static club.mcams.entityrngcracker.translations.TranslationConstants.*;
...
public class CarpetExtensionTranslations
{
	...
	{
		return translate(text, ((ServerPlayerEntityWithClientLanguage)player).getClientLanguage$CME());
	}
	...
	private static BaseText translateText(BaseText text, @NotNull String lang)
	{
		...
		return forEachTISCMTranslationText(Messenger.copy(text), lang, (txt, msgKeyString) -> {
			...
			{
				CrackerServer.LOGGER.warn("TISCM: Unknown translation key {}", txtKey);
				return txt;
			}
